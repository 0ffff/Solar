...
namespace MX100VCS
{
    public class DrawWaveMX100 : DAQMX100
    {
        ...
        public void RunDraw(TextBox address, PictureBox pic, Label vallabel)
        {
            ...
            int comm = openMX100(enc.GetBytes("192.168.1.5"), out rc);
            int chNo = channelNumberMX100(comm, fifoNo, INDEXNO);

            rc = DAQMX100.measStartMX100(comm);
            do
            {
                rc = DAQMX100.measDataFIFOMX100(comm, fifoNo);
                if (DAQMX100.dataValidMX100(comm, chNo) == DAQMX100.DAQMX100_VALID_ON)
                {
